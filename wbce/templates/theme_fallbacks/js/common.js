...
$(document).ready(function () {
    /**
     *  apply chosen jQuery Plugin to select elements
     */
    if ($("select.chosen").length) {
        $.insert(JQUERY_PLUGINS + '/chosen/chosen.min.css');
        $.insert(JQUERY_PLUGINS + '/chosen/chosen.jquery.min.js');
        // initiate chosen for select elements
        $("select.chosen").chosen({
            disable_search_threshold: 5 //show the search bar in select if > 7 elements
        });
    }
